  ...
  describe('.createRouter', () => {
    ...
    describe('router', () => {
      ...
      it('returns 404 when invalid route', async () => {
        ...
        expect(send).toHaveBeenCalledWith(res, 404, expect.anything());
      });
      ...
      it('logs request', async () => {
        ...
        expect(logger.info).toHaveBeenCalledWith('Incoming request', { req: expected });
      });
      ...
      it('returns application info for /health', async () => {
        ...
        const res = { setHeader: jest.fn() };
        const expected = { name: 'Unknown', description: '', host, dependencies: [], version: 'development' };
